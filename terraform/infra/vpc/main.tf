...
### create NAT gateway, EIP and private subnetes route table
resource "aws_eip" "nat_eip" {
  ...
  depends_on = [aws_internet_gateway.opentelemetry_igw]
}

# allocate the nat gateway in the first public subnet
resource "aws_nat_gateway" "nat_gw" {
  allocation_id = aws_eip.nat_eip.id
  subnet_id = aws_subnet.opentelemetry_public_subnet[0].id
  tags = {
...
##create route table for private subnets and route the traffic to NAT gateway
...
resource "aws_route_table_association" "private_subnet_route_association" {
  count     = length(var.private_subnet_cidrs)
  subnet_id = aws_subnet.opentelemetry_private_subnet[count.index].id
